...
# #--- geometric ---
def do_random_rotate(image, mag=15 ):
...
def do_random_scale( image, mag=0.1 ):
...
def do_random_stretch_y( image, mag=0.25 ):
    s = 1 + np.random.uniform(-1, 1)*mag
    height, width = image.shape[:2]
    h = int(s*height)
    w = width
...
def do_random_stretch_x( image, mag=0.25 ):
    s = 1 + np.random.uniform(-1, 1)*mag
    height, width = image.shape[:2]
    h = height
    w = int(s*width)
...
def do_random_shift( image, mag=32 ):
    b = mag
    height, width = image.shape[:2]
